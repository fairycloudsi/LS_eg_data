...
%% 1. Generate UDV matrices
for i = 1:size(d,1)
    U = orth(randn(M(i),N(i)));
    V = orth(randn(N(i),N(i)));
    A = U*diag(linspace(1,c(i),N(i)))*V';
    x_ect = randn(N(i),1);
    b{i} = A*x_ect;
    UDV{i} = A;
...
%% 2. Save UDV matrices 
filename = strcat('data/LS_UDV_lambda01.mat');
save(filename,'UDV','b');

%% 3. Amg solve L2 regularized problem compute the condition number of Hessian 
data = load(filename);
...
%% 4. Save f_opt and condat_list
filename = strcat('data/LS_UDV_f_opt_lambda01.mat');
save(filename,'f_opt');
filename = strcat('data/LS_UDV_conda_list_lambda01.mat');
save(filename, 'conda_list')

%% 5. Use rFASD and rCD to solve and compare results 
